...
| 5. Stories/Fleets | Lets users post ephemeral content that disappears after 24 hours. Integrated into the Explore page. | To Be Refactored | `src/app/(app)/explore/page.tsx`, `src/app/(app)/status/components/create-status-dialog.tsx`, `src/app/(app)/status/components/view-status-dialog.tsx` |
...
| 7. User Profiles | Dedicated pages for each user, showcasing their bio, posts, followers, and other activity. | Completed | `src/app/(app)/profile/[username]/page.tsx` |
...
| 13. Events | Allows users to create and manage events, with options for RSVPs and sharing. | Completed | `src/app/(app)/events/[id]/page.tsx`, `src/app/(app)/events/components/create-event-dialog.tsx` |
...
| 15. Lists | Users can create custom lists of other users to view a specific timeline of posts from just those accounts. | Not Started | new file `src/app/(app)/lists/page.tsx` |
...
| 31. Mood Board | A visual feature allowing users to create and share collages of images, text, and links that represent their current mood or inspiration. | Not Started | new file `src/app/(app)/moodboard/page.tsx` |
| 32. Challenges | A feature for creating or participating in community-wide challenges or trends, often centered around a specific hashtag. | Not Started | new file `src/app/(app)/challenges/page.tsx` |
...
| 44. Backend | The core server-side logic, database schema, and serverless functions that power the entire application. | In Progress | `supabase/schema.sql`, `supabase/functions.sql` |

### Monetization / Promotions Integration Plan

This is an excellent next step. Integrating monetization requires careful planning. Here is a comprehensive, phased approach based on your ideas.

#### **Phase 1: Foundational Setup & Google Ads**

The goal here is to get the basic ad infrastructure in place for the platform.

1.  **Prerequisite:** Create a [Google AdSense](https://www.google.com/adsense/start/) account. This is an external step you will need to complete.
2.  **Integrate AdSense Script:** Add the core AdSense script to the entire application.
    *   **File:** `src/app/layout.tsx`
3.  **Develop Ad Component:** Create a reusable component to display Google Ads.
    *   **File:** `src/components/ads/google-ad.tsx` (new file)
4.  **Place Ads:** Strategically place the `GoogleAd` component in high-traffic areas.
    *   **File:** `src/app/(app)/page.tsx`

---

#### **Phase 2: Creator-Facing Promotions**

This phase focuses on building the tools for users to promote their own content.

1.  **Database Schema Update:** We need to update the database to track promoted posts.
    *   **File:** `supabase/schema.sql`
    *   **Action:** I will add a `promoted_posts` table to store information like `post_id`, `promotion_level`, `start_date`, and `end_date`.
2.  **"Boost Post" UI:**
    *   **File:** `src/app/(app)/components/post-card.tsx`
    *   **Action:** Add a "Boost Post" button to the post options menu.
    *   **File:** `src/app/(app)/components/promote-post-dialog.tsx`
    *   **Action:** This dialog will present boosting options and pricing tiers.
3.  **Promotions Hub:** Build the main settings page where creators can manage their post promotions.
    *   **File:** `src/app/(app)/settings/promotions/page.tsx`
4.  **Update Feed Algorithm:**
    *   **File:** `src/app/(app)/page.tsx` (or the relevant feed-generating file)
    *   **Action:** Adjust the logic that fetches posts to inject boosted posts at a higher priority.

---

#### **Phase 3: Payment Integration with Stripe**

To enable post boosting, we need a secure way to handle payments. Stripe is the industry standard for this.

1.  **Prerequisite:** Create a [Stripe](https://stripe.com/) account.
2.  **Install Stripe Libraries:** We'll need to add the necessary Stripe packages to the project.
    *   **Command:** `npm install @stripe/stripe-js @stripe/react-stripe-js`
3.  **Create Secure Backend Functions:** We will create Supabase Edge Functions to securely handle payment intent creation with Stripe's API. This keeps sensitive keys off the client-side.
    *   **File:** `supabase/functions/create-payment-intent/index.ts` (new file)
4.  **Build Checkout Experience:** Integrate Stripe's payment elements into the `PromotePostDialog`.
    *   **File:** `src/app/(app)/components/promote-post-dialog.tsx`
    *   **Action:** Add the Stripe `CardElement` to securely collect payment information and handle the transaction process.
