...
export function EventCard({ event, onRsvp }: { event: Event, onRsvp: () => void }) {
    ...
    // Summarize RSVP list (no global user list needed — only count is shown in tooltip)
    const rsvpCount = (status: RSVPStatus) =>
        event.rsvps?.filter(r => r.status === status).length || 0;
