  ...
  const { t } = useTranslation();
    ...
    return (
        <div className="flex flex-col items-center justify-center h-[100dvh] bg-black text-white p-6 text-center">
            ...
            <h2 className="text-2xl font-bold mb-2">{t('live.streamError')}</h2>
            ...
            <div className="flex flex-col gap-3 w-full max-w-xs">
                {onRetry && (
                    ...
                    >
                        <RefreshCcw className="mr-2 h-4 w-4" />{t('live.tryAgain')}</Button>
                )}

                <Link href="/live" className="w-full">
                    <Button variant="outline" className="w-full border-white/20 text-white hover:bg-white/10">
                        <Home className="mr-2 h-4 w-4" />{t('live.goToLiveHub')}</Button>
                </Link>
            </div>
        </div>
    )
