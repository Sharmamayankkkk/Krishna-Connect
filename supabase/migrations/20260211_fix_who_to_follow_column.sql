-- Fix for get_who_to_follow column name error
-- It was using target_user_id (from reports) but should be user_two_id (from relationships)
...
BEGIN
    ...
    FROM (
        ...
        FROM profiles p
        WHERE p.id != auth.uid()
        -- FIX: Changed target_user_id to user_two_id
        AND p.id NOT IN (SELECT user_two_id FROM public.relationships WHERE user_one_id = auth.uid() AND status = 'approved')
        ORDER BY RANDOM()
        LIMIT limit_count
    ) t;
