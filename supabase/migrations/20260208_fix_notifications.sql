-- ============================================================================
-- 20260208_fix_notifications.sql
-- Description: Fix notification links by including post author username
-- ============================================================================
...
BEGIN
    ...
    SELECT 
        ...
        CASE WHEN n.type IN ('like', 'comment', 'repost', 'quote', 'collaboration_request', 'mention') AND n.entity_id IS NOT NULL THEN SUBSTRING(posts.content FROM 1 FOR 100) ELSE NULL END,
        CASE WHEN n.type IN ('like', 'comment', 'repost', 'quote', 'collaboration_request', 'mention') AND n.entity_id IS NOT NULL AND jsonb_array_length(posts.media_urls) > 0 THEN posts.media_urls->0->>'type' ELSE NULL END,
